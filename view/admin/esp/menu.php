<div id="wrapper-esp-all">
			        <div class="menu-div-esp">
            <ul class="menu-esp-ul">
              <li><a href="?c=esp&a=Index">Listar Espectaculos</a></li>
              <li class="submenu"><a href="#">Agregar Nuevo</a>
              <ul class="menu-esp-ul"> 	
              <form method="POST" action="?c=esp&a=Guardar" id="form" enctype="multipart/form-data">
		<li><input type="hidden" name="id"/></li>
		<li>Nombre:</li>
		<li><input type="text" name="nombre" placeholder="Ingrese el Nombre" required/></li>
		<li>Titulo:</li>
		<li><input type= "text" name="artista" placeholder="Ingrese EL Slogan" required/></li>
		<li>Logo:</li>
		<li><input type="file" name="img"/></li>
		<li>Banner:</li>
		<li><input type="file" name="banner"/></li>
		<li>Descripcion:</li>
		<li><textarea rows="10" cols="50" name="descripcion" form="form"></textarea></li>
		<li><input class="button edit" type="submit" value="Registrar"/></li>
		</form>
				
			</ul>
		</li>
            </ul>
            		</div>
